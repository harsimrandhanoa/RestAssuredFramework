...
//we are passed a colNum and rowNum as integers for that.
	...
	public  String getCellData(String sheetName,int colNum,int rowNum){
		try{
			...
			//So we first check if colNum passed is not -1 or not less than 0 .If yes return empty String			
			if(colNum<0)
				return "";
	...
	//Maybe we are checking because we are passing row no starting from 1 in called method
    //But in actual excel file row starts from 0.
	//We are checking if row exists. If not return empty String. But why checking rowNum-1 . No idea.
   ...
   //if its null that is col does not exist then return empty String	
		//Same logics for colNum-1 as in calling method we pass col num as 1 ,but in excel reader we read
		//from col 0
		...
		// Then once we have check that row object and cell object are not null ,then we get the data and if data 
		 //is String return String, if its numeric return numeric, if its Blank return blank.		
