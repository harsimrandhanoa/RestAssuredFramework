...
public class AddProduct extends BaseTest {
	...
	public void addProduct(Hashtable<String, String> data) {
		...
		Response resp = given().filter(new RequestLoggingFilter(requestCapture)).contentType(ContentType.JSON)
				.headers("sessionid", sessionId).log().body().body(prod).post();

		// Response resp = given().filter(new
		// RequestLoggingFilter(requestCapture)).contentType(ContentType.JSON).headers("sessionid",sessionId).log().all().when().body(cc).post();
		...
		if (!actualStatus.equals("product added successfully")) {
			...
			reportFailure("Failed to add product  named   " + productName + " under cateogory" + categoryName
					+ " to db.The status is --> " + actualStatus + " and the error message we are getting is --> "
					+ errorMessage, false);
